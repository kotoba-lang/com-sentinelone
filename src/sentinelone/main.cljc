(ns sentinelone.main
  "Kotodama WASM entrypoint for the Sentinelone clean-room actor (L4) — Clojure port.
  ...
  facts (`sentinelone.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sentinelone")
(def tier "L4")
...
(def entity-specs
  [{:entity "Identity"  :plural "identities"  :id-prefix "sentinel_ide"
    :fields [:username :email :status :mfaEnabled]           :required [:username :email]
    :coerce {:mfaEnabled :bool}              :refs {}}
   {:entity "Policy"    :plural "policies"    :id-prefix "sentinel_pol"
    :fields [:name :effect :resources :actions]              :required [:name :effect]
    :coerce {}                               :refs {}}
   {:entity "Detection" :plural "detections" :id-prefix "sentinel_det"
    :fields [:severity :rule :entityRef :detectedAt]        :required [:severity :rule]
    :coerce {}                               :refs {}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "sentinel_inc"
    :fields [:title :severity :status :assignee]             :required [:title :severity]
    :coerce {}                               :refs {}}
   {:entity "Asset"     :plural "assets"     :id-prefix "sentinel_ass"
    :fields [:hostname :ipAddress :os :riskScore]            :required [:hostname :ipAddress]
    :coerce {:riskScore :float}              :refs {}}
   {:entity "Session"   :plural "sessions"   :id-prefix "sentinel_ses"
    :fields [:identityId :source :expiresAt]                 :required [:source :expiresAt]
...
(defn emit-facts
  "EAVT facts for one record: {\"sentinelone.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sentinelone-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sentinelone-compat/ready)
